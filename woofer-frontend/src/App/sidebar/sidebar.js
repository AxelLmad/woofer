...
class Sidebar extends React.Component{
    ...
    render(){
        ...
        return(
            <div>
                <button onClick={this.handleClickMenuButton} className={"bg-light w-8 h-8 ml-2 mt-2 pt-1 active:bg-primary outline-none md:hidden"}>
                    <img className={"transform scale-150 mx-auto"} src={menuIcon} alt="Menu"/>
                </button>
                <nav className={`md:flex flex-col text-light w-screen md:w-72 bg-midnight h-screen font-semibold pt-8
                 ${this.state.menuButtonFlag?'flex':'hidden'}`}>
                    ...
                    <Link className={"pl-8 py-4 flex flex-row hover:bg-primary hover:text-midnight"} to={"/"}>
                        <span>Comfiguración</span> <img className={"w-6 ml-4"} src={configIcon} alt="configuracion"/>
                    </Link>
